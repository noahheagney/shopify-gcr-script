...
function useSelectedPaymentOptions() {
  const selectedPaymentOptions = useSubscription(useApi().selectedPaymentOptions);
  const availablePaymentOptions = useSubscription(useApi().availablePaymentOptions);
  return useMemo(() => {
